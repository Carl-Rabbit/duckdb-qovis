...
public:
    ...
    static yyjson_mut_val *RenderRecursive(yyjson_mut_doc *doc, const LogicalOperator &op) {
        auto object = yyjson_mut_obj(doc);
        auto children = yyjson_mut_arr(doc);

        for (auto &child : op.children) {
            auto child_object = RenderRecursive(doc, *child);
        ...
        yyjson_mut_obj_add_strcpy(doc, object, "params", op.ParamsToString().c_str());
        return object;
    }
    ...
    static void LogStepStart(const std::string& stepName, const std::string& procType, const LogicalOperator &op) {
        ...
        auto plan = RenderRecursive(doc, op);
        yyjson_mut_obj_add_val(doc, root, "plan", plan);
